...
<body>
  ...
  <!-- Three.js - Using newer version that has CapsuleGeometry -->
  ...
  <script type="module">
    ...
    function createPlayer() {
      ...
      // Hair
      const hairGeom = new THREE.SphereGeometry(0.38, 16, 8, 0, Math.PI * 2, 0, Math.PI / 2);
    ...
    function createPhotoFrame(x, y, z, id, title, photoUrl) {
      ...
      interactables.push({
        ...
        id: id,
        title: title,
        content: photoUrl ? '<img src="' + photoUrl + '" alt="Memory">' : '<p style="font-size:60px">📸</p><p>A beautiful photo of us</p>',
        type: 'photo',
        collected: false
      });
    }
    ...
    function createGiftBox(x, y, z, id, title, message) {
      ...
      // Bow (simple spheres instead of torus)
      const bowGeom = new THREE.SphereGeometry(0.12, 16, 16);
      const bow1 = new THREE.Mesh(bowGeom, ribbonMat);
      bow1.position.set(-0.15, 0.35, 0);
      bow1.scale.set(1, 0.7, 1);
      group.add(bow1);

      const bow2 = new THREE.Mesh(bowGeom, ribbonMat);
      bow2.position.set(0.15, 0.35, 0);
      bow2.scale.set(1, 0.7, 1);
      group.add(bow2);

      const bowCenter = new THREE.Mesh(new THREE.SphereGeometry(0.08, 16, 16), ribbonMat);
...
async function update3DTemplate() {
  console.log('🔧 Updating 3D Memory Room template...');
  
  const result = await db
    .update(templates)
    .set({ htmlTemplate: updatedHtml })
    .where(eq(templates.name, '3D Memory Room'))
    .returning();
  ...
  if (result.length > 0) {
    ...
    const newResult = await db.insert(templates).values({
      name: '3D Memory Room',
      description: 'A stunning 3D room experience where your special someone explores a beautiful space filled with floating memories, photo frames, and hidden love messages.',
    ...
    console.log('✅ Created template:', newResult[0].name);
  }
  
  process.exit(0);
}

update3DTemplate().catch(err => {
